...
from .aws import Csstatus,CScommand,CurrentPlayerCount
...
class tournamentCreate(LoginRequiredMixin,CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['registration_starts'].widget = forms.TextInput(attrs={'placeholder': 'YYYY-MM-DD'})
        form.fields['registration_ends'].widget = forms.TextInput(attrs={'placeholder': 'YYYY-MM-DD'})
        form.fields['tournament_starts'].widget = forms.TextInput(attrs={'placeholder': 'YYYY-MM-DD'})
        form.fields['tournament_ends'].widget = forms.TextInput(attrs={'placeholder': 'YYYY-MM-DD'})
...
class tournamentDetails(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.get_object().registration_starts <= d and self.get_object().registration_ends >= d:
            context["status"] = "Registration started"

        if d > self.get_object().registration_ends:
            context["status"] = "Registration ended"
           

        if self.get_object().tournament_starts <= d and self.get_object().tournament_ends >= d:
            context["status"] = "Ongoing"
            
        if self.get_object().tournament_ends <= d:
...
class serverDetails(DeleteView,LoginRequiredMixin):
    ...
    def get_context_data(self,**kwargs):
        self.temp = self.get_object()
        if self.request.user == self.temp.tournament_set.first().organizer:
            ...
            context["players"] = CurrentPlayerCount(obj.ip)
